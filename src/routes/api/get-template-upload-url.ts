...
export const ServerRoute = createServerFileRoute("/api/get-template-upload-url").methods({
  POST: async ({ request }) => {
    ...
    try {
      ...
      const { path, expirationSeconds = 300 } = body;
      ...
      // Request presigned URL from Atlas for template upload
      ...
      console.error("Get template upload URL error:", error);
      return new Response(
        error instanceof Error ? error.message : "Failed to get template upload URL",
        { status: 500 }
      );
    }
  },
